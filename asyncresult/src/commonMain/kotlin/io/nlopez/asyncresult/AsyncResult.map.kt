...
/**
 * Converts [Loading] and [NotStarted] to a [AsyncResult] of type [T]. This is to prevent us to having to
 * map everything when we know we don't have a [Success] value, where types would actually matter.
 */
inline fun <T> Incomplete.cast(): AsyncResult<T> = when (this) {
    Loading -> Loading
    NotStarted -> NotStarted
}

/**
 * Converts the current [AsyncResult] to a [AsyncResult] of type [R] if the current value is of type [R].
 * If the current value is not of type [R], it will return an [Error].
 */
...
/**
 * Transforms the current [AsyncResult] to the result of [transform] if it's a [Success], based on its contained
 * value.
 */
...
/**
 * Transforms the type of [AsyncResult] from nullable to non-nullable. If the value is null, it will return an
 * [Error]. It also allows adding specific metadata to disambiguate errors if necessary, via [lazyMetadata].
 */
inline fun <reified R> AsyncResult<R?>.orError(
    noinline lazyMetadata: (() -> Any?)? = null,
): AsyncResult<R> = when (this) {
    NotStarted -> NotStarted
    Loading -> Loading
 ...
 */
fun <A, B> AsyncResult<Pair<A, B>>.spread(): Pair<AsyncResult<A>, AsyncResult<B>> = when (this) {
...
inline operator fun <T> AsyncResult<Pair<T, *>>.component1(): AsyncResult<T> = mapSuccess { it.first }

@JvmName("component2Pair")
inline operator fun <T> AsyncResult<Pair<*, T>>.component2(): AsyncResult<T> = mapSuccess { it.second }

@JvmName("component1Triple")
inline operator fun <T> AsyncResult<Triple<T, *, *>>.component1(): AsyncResult<T> = mapSuccess { it.first }

@JvmName("component2Triple")
inline operator fun <T> AsyncResult<Triple<*, T, *>>.component2(): AsyncResult<T> = mapSuccess { it.second }

@JvmName("component3Triple")
inline operator fun <T> AsyncResult<Triple<*, *, T>>.component3(): AsyncResult<T> = mapSuccess { it.third }
