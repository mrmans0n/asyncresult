...
inline fun <R> AsyncResult<R>.unwrap(): R {
    contract {
        returns() implies (this@unwrap is Success)
    }
...
inline fun AsyncResult<*>.unwrapError(): Error {
    contract {
        returns() implies (this@unwrapError is Error)
    }
...
/**
 * Extracts the value from the [AsyncResult] if it's an [Error] with a [Throwable],
 * otherwise throws an [UnwrapException].
 */
inline fun AsyncResult<*>.unwrapThrowable(): Throwable {
    contract {
        returns() implies (this@unwrapThrowable is Error)
    }

    val error = errorOrNull() ?: throw UnwrapException("Tried to unwrap a non-`Error` value")
    return error.throwable ?: throw UnwrapException("Tried to unwrap an `Error` that had no Throwable")
}

inline fun <reified M> AsyncResult<*>.unwrapMetadata(): M {
    contract {
        returns() implies (this@unwrapMetadata is Error)
    }

    return errorWithMetadataOrNull<M>() ?: throw UnwrapException("Tried to unwrap a non-`Error` value")
}

/**
 * Extracts the value from the [AsyncResult] if it's a [Success], otherwise throws an [UnwrapException]
 * with the computed [message] in it.
 */
...
/**
 * Extracts the value from the [AsyncResult] if it's an [Error] with a [Throwable] in it,
