...
inline fun <R> AsyncResult<R>.errorOrNull(): Error? {
    contract {
        returns() implies (this@errorOrNull is Error)
    }
    return this as? Error
}

/**
 * Returns the metadata value from the [Error] if the [AsyncResult] is [Error], and has metadata
 * or the requested type.
 */
inline fun <reified M> AsyncResult<*>.errorWithMetadataOrNull(): M? = errorOrNull()?.metadataOrNull<M>()
 ...
 * Returns the value if the [AsyncResult] is [Success] else returns null.
 */
inline fun <R> AsyncResult<R>.getOrThrow(): R = when (this) {
    is Success -> value
    else -> throw throwableOrNull() ?: IllegalArgumentException("AsyncResult does not contain value")
}
