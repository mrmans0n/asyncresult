...
inline fun <T, reified M> Assert<AsyncResult<T>>.isErrorWithMetadata(): Assert<M> =
    transform { actual ->
      when (actual) {
        is Error ->
...
inline fun <T, reified M> Assert<AsyncResult<T>>.isErrorWithMetadataEqualTo(value: M) =
    given { actual ->
